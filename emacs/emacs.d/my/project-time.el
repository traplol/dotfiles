...
Time spent idle is not tracked.
  ...
  (let ((sorted (sort (pt--consolidate-all-times-to-dirs)
    ...
    (dolist (entry sorted)
      (let ((dir (car entry))
        ...
        (message (format "%s %s" dir (pt--format-timespan time-total)))))))
...
PT--TIME-OF-LAST-SAVE to the current time."
  ...
  (save-excursion
    (let ((buf (find-file-noselect project-time-save-file)))
      (set-buffer buf)
      (erase-buffer)
      (princ ";;; This is an autogenerated file.\n;;; Any modifications to this file will probably be lost." buf)
      (princ "\n(" buf)
      (dolist (entry pt--buffer-times) (print entry buf))
      (princ "\n)" buf)
      (save-buffer))))
...
(define-minor-mode project-time-mode
  "minor mode for keeping track of time spend editing buffers."
  :global t
  :lighter " PT"
  nil
  (progn
